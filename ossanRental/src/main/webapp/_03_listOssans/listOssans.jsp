...
<!-- 			Jay:注意這邊的用法，可以在迴圈裡用兩種變數 -->
        ...
        <tr>
        	<td>
		        <c:if test="${pageNo > 1}">
		           <div id="pfirst">
		              <a href="<c:url value='DisplayOssanProducts?
        	...
        	<td>
		        <c:if test="${pageNo > 1}">
		           <div id="pprev">
		              <a href="<c:url value='DisplayOssanProducts?pageNo=${pageNo-1}&requestArea=${sessionScope.sessionArea}' />">上一頁</a>
		           </div>
		        </c:if>       
        	</td>
        	<td>
	            <c:if test="${pageNo != totalPages}">
	                <div id="pnext">
	                   <a href="<c:url value='DisplayOssanProducts?pageNo=${pageNo+1}&requestArea=${sessionScope.sessionArea}'/>">下一頁</a>
	                </div>
	            </c:if>       	
        	</td>
        	<td>
	            <c:if test="${pageNo != totalPages}">
	                <div id="plast">
	                    <a href="<c:url value='DisplayOssanProducts?pageNo=${totalPages}&requestArea=${sessionScope.sessionArea}' />">最末頁</a>
	                </div>
	            </c:if>        	
        	</td>
        	<td>
